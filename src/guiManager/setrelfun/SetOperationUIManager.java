...
import graphicAnimation.AnimationArea;
import guiManager.ImagedTextAreaAnimator;
 ...
 * 用于演示集合运算的界面管理器
 */
public class SetOperationUIManager extends JPanel {
	...
	private JCheckBox differenceBox = null;
	...
	public SetOperationUIManager(PlainTextAreaManager plainManager, ImagedTextAreaManager imageManager) {
	...
	public boolean setAllSets(String setUString, String setAString, String setBString) {
	...
	public void setAllOperators() {
		...
		differenceBox.setSelected(true);
	...
	private JPanel createInputSetPanel() {
		...
		// 设置该窗格使用的带标题的边框
 		inputPanel.setBorder(BorderFactory.createTitledBorder("请输入集合的元素，可用也可不用左右花括号括住："));
	...
	private JPanel createButtonPanel() {
		...
		infoPanel.setBorder(BorderFactory.createTitledBorder("选择要进行的集合运算："));
		intersectionBox = new JCheckBox("集合交");
		infoPanel.add(intersectionBox);
		unionBox = new JCheckBox("集合并");
		infoPanel.add(unionBox);
		subtractBox = new JCheckBox("集合差");
		infoPanel.add(subtractBox);
		complementBox = new JCheckBox("绝对补");
		infoPanel.add(complementBox);
		differenceBox = new JCheckBox("对称差");
		infoPanel.add(differenceBox);
		powerBox = new JCheckBox("集合幂");
	...
	private class SimpleListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent evt) {
			...
			if (command.equals("ok")) {
				...
				differenceBox.setSelected(false);
				...
				setAllSets(setUString, setAString, setBString);
				intersectionBox.setSelected(true);
				unionBox.setSelected(false);
				subtractBox.setSelected(false);
				complementBox.setSelected(false);
		...
		public boolean doSetOperation() {
			...
			if (intersectionBox.isSelected()) {
				...
				textAreaManager.appendContent("\tA\\cap B = " + result.toString() + "\n");
				imagedAreaManager.appendPlainStringAsNewLine("    两集合的交: ");
				imagedAreaManager.appendLaTeXStringToLastLine("A\\cap B = " + result.toLaTeXString());
			}
			if (unionBox.isSelected()) {
				Set result = setA.union(setB);
				result = setU.getSubsetForCharFunction(setU.getCharFunctionForSubset(result));
				textAreaManager.appendContent("\tA\\cup B = " + result.toString() + "\n");
				imagedAreaManager.appendPlainStringAsNewLine("    两集合的并: ");
				imagedAreaManager.appendLaTeXStringToLastLine("A\\cup B = " + result.toLaTeXString());
			}
			if (subtractBox.isSelected()) {
				Set result = setA.subtract(setB);
				result = setU.getSubsetForCharFunction(setU.getCharFunctionForSubset(result));
				textAreaManager.appendContent("\tA - B = " + result.toString() + "\n");
				imagedAreaManager.appendPlainStringAsNewLine("    两集合的差: ");
				imagedAreaManager.appendLaTeXStringToLastLine("A - B = " + result.toLaTeXString());
			}
			if (complementBox.isSelected()) {
				Set result = setU.subtract(setA);
				result = setU.getSubsetForCharFunction(setU.getCharFunctionForSubset(result));
				textAreaManager.appendContent("\t\\overline{A} = " + result.toString() + "\n");
				imagedAreaManager.appendPlainStringAsNewLine("    集合绝对补: ");
				imagedAreaManager.appendLaTeXStringToLastLine("\\overline{A} = " + result.toLaTeXString());
				result = setU.subtract(setB);
				result = setU.getSubsetForCharFunction(setU.getCharFunctionForSubset(result));
				textAreaManager.appendContent("\t\\overline{B} = " + result.toString() + "\n");
				imagedAreaManager.appendPlainStringAsNewLine("    集合绝对补: ");
				imagedAreaManager.appendLaTeXStringToLastLine("\\overline{B} = " + result.toLaTeXString());
			}
			if (differenceBox.isSelected()) {
				Set result = setA.symmetricDifference(setB);
				result = setU.getSubsetForCharFunction(setU.getCharFunctionForSubset(result));
				textAreaManager.appendContent("\tA\\oplus B = " + result.toString() + "\n");
				imagedAreaManager.appendPlainStringAsNewLine("    集合对称差: ");
				imagedAreaManager.appendLaTeXStringToLastLine("A\\oplus B = " + result.toLaTeXString());
			}
			if (powerBox.isSelected()) {
				Set[] result = setA.powerSet();
				if (result.length == 1) textAreaManager.appendContent("\t\\wp(A) = \\{  " + result[0].toString());
				else textAreaManager.appendContent("\t\\wp(A) = \\{  " + result[0].toString() + ", ");
				imagedAreaManager.appendPlainStringAsNewLine("    集合的幂集: ");
				if (result.length == 1) imagedAreaManager.appendLaTeXStringToLastLine("\\wp(A) = \\{\\quad " + result[0].toLaTeXString());
				else imagedAreaManager.appendLaTeXStringToLastLine("\\wp(A) = \\{\\quad " + result[0].toLaTeXString() + ", \\quad ");
				for (int i = 1; i < result.length; i++) {
					if (i % 8 == 0) {
						if (i < result.length-1) textAreaManager.appendContent("\n\t\t" + result[i].toString() + ", ");
						else textAreaManager.appendContent("\n\t\t" + result[i].toString());
					} else {
						if (i < result.length-1) textAreaManager.appendContent(result[i].toString() + ", ");
						else textAreaManager.appendContent(result[i].toString());
					}
					
					if (i % 8 == 0) {
						imagedAreaManager.appendPlainStringAsNewLine("                        ");
						if (i < result.length-1) imagedAreaManager.appendLaTeXStringToLastLine(result[i].toLaTeXString() + ", \\quad ");
						else imagedAreaManager.appendLaTeXStringToLastLine(result[i].toLaTeXString());
					} else {
						if (i < result.length-1) imagedAreaManager.appendLaTeXStringToLastLine(result[i].toLaTeXString() + ", \\quad ");
						else imagedAreaManager.appendLaTeXStringToLastLine(result[i].toLaTeXString());
					}
				...
				imagedAreaManager.appendLaTeXStringToLastLine("\\quad\\quad\\}");

				result = setB.powerSet();
				if (result.length == 1) textAreaManager.appendContent("\t\\wp(B) = \\{  " + result[0].toString());
				else textAreaManager.appendContent("\t\\wp(B) = \\{  " + result[0].toString() + ", ");
				imagedAreaManager.appendPlainStringAsNewLine("    集合的幂集: ");
				if (result.length == 1) imagedAreaManager.appendLaTeXStringToLastLine("\\wp(B) = \\{\\quad " + result[0].toLaTeXString());
				else imagedAreaManager.appendLaTeXStringToLastLine("\\wp(B) = \\{\\quad " + result[0].toLaTeXString() + ", \\quad ");
				for (int i = 1; i < result.length; i++) {
					if (i % 8 == 0) {
						if (i < result.length-1) textAreaManager.appendContent("\n\t\t" + result[i].toString());
						else textAreaManager.appendContent("\n\t\t" + result[i].toString());
		...
		public boolean generateSets() {
			...
			setAllSets(setU.toString(), setA.toString(), setB.toString());
			return true;
		}
