...
import guiManager.ImagedTextAreaManager;
import guiManager.LaTeXFormulaInputManager;
import guiManager.MainSwingFrame;
import guiManager.PlainTextAreaManager;
import proplogic.FormulaBuilder;
...
import proplogic.normalFormula.ExpandNFRecorder;
import proplogic.normalFormula.ExpandNFStep;
...
import setrelfun.SetrelfunUtil;

/**
 * @author Zhou Xiaocong
 * @since 10/06/2020
 * 
 * 与用户进行交互，输入一个或多个简单析取式（或简单合取式），然后扩展为主合取范式或主析取范式
 */
public class NormalFormulaExpansionUIManager {
	...
	private JRadioButton pdnfButton = null;
	private JRadioButton pcnfButton = null;
	private int counter = 1;

	public NormalFormulaExpansionUIManager(PlainTextAreaManager plainManager, ImagedTextAreaManager imageManager) {
	...
	public void setToBeDFN(boolean isDFN) {
		if (isDFN) pdnfButton.setSelected(true);
		else pcnfButton.setSelected(true);
	}
	...
	private JPanel createButtonPanel() {
		...
		// 创建用于确定输入信息的“确定”按钮
		JButton actButton = new JButton("扩展主范式(Y)");
		...
		infoPanel.setBorder(BorderFactory.createTitledBorder("扩展范式所需的相关信息"));
		...
		// 创建一个标签用于提示输入性别信息
		promptLabel = new JLabel("将它们扩展为：", JLabel.RIGHT);
		// 创建用于将下两个广播按钮捆绑成一组的按钮组对象，同一组的广播按钮只能选中一个，
		// 不同组的广播按钮则可以同时选中不同的广播按钮
		ButtonGroup group = new ButtonGroup();
		// 创建选择性别的广播按钮
		pdnfButton = new JRadioButton("主析取范式");
		pdnfButton.setSelected(true);		// 缺省时选中该广播按钮
		group.add(pdnfButton);
		pcnfButton = new JRadioButton("主合取范式");
		group.add(pcnfButton);
		tempPanel.add(promptLabel);
		tempPanel.add(pdnfButton);			// 注意是添加广播按钮组件
		tempPanel.add(pcnfButton);		// 而不是添加按钮组对象
		infoPanel.add(tempPanel);
	...
	private class SimpleListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent evt) {
			...
			if (command.equals("ok")) {
				...
				boolean isDNF = true;
				if (pcnfButton.isSelected()) isDNF = false;
				List<String> laTeXStringList = inputManager.getFormulaList();
				expandNormalFormula(variableSetString, laTeXStringList, isDNF);
				counter = counter + 1;
		...
		private boolean expandNormalFormula(String variableSetString, List<String> laTeXStringList, boolean isDNF) {
			...
			String wholeString = NormalFormulaExpansionUIManager.getWholeString(laTeXStringList, isDNF);
			...
			if (isDNF) {
				...
				if (disjnormform == null) {
					textAreaManager.appendContent(counter + ": Formula " + formula.toSimpleLaTeXString() + " is not disjunctive normal formula!\n");
					...
					imagedAreaManager.appendPlainStringToLastLine(" 不是析取范式，无法扩展为主析取范式!");
					
					JOptionPane.showMessageDialog(inputManager.getDialog(), "输入的字符串 " + formula.toSimpleLaTeXString() + " 整体上不是析取范式，无法扩展!", "离散数学基础例题习题展示", JOptionPane.WARNING_MESSAGE);
					return false;
				}
				PDNFormula lastPDNF = NormalFormulaCalculator.expandToPDNF(disjnormform, varSet);

				textAreaManager.appendContent(counter + ": Expand disjunctive normal formula " + formula.toSimpleLaTeXString() + "\n");
				imagedAreaManager.appendPlainStringAsNewLine(counter + ": 扩展析取范式 ");
				imagedAreaManager.appendLaTeXStringToLastLine(formula.toSimpleLaTeXString());
				...
				for (ExpandNFStep step : stepList) {
					formula = step.getFormula();
					inputManager.appendLaTeXStringAsNewFeedbackLine(formula.toSimpleLaTeXString());
						
					textAreaManager.appendContent("\tExpand " + formula.toSimpleLaTeXString() + "[" + step.getFormulaBinaryCodeString() + "] to min terms: " + step.getResultCodesNamedLaTeXString(type) + "\n");
				...
				if (conjnormform == null) {
					textAreaManager.appendContent(counter + ": Formula " + formula.toSimpleLaTeXString() + " is not conjunctive normal formula!\n");
					imagedAreaManager.appendPlainStringAsNewLine(counter + ": 公式 ");
					imagedAreaManager.appendLaTeXStringToLastLine(formula.toSimpleLaTeXString());
					imagedAreaManager.appendPlainStringToLastLine(" 不是合取范式，无法扩展为主合取范式!");

					JOptionPane.showMessageDialog(inputManager.getDialog(), "输入的字符串 " + formula.toSimpleLaTeXString() + " 整体上不是合取范式，无法扩展!", "离散数学基础例题习题展示", JOptionPane.WARNING_MESSAGE);
					return false;
				}
				PCNFormula lastPCNF = NormalFormulaCalculator.expandToPCNF(conjnormform, varSet);
				
				textAreaManager.appendContent(counter + ": Expand conjunctive normal formula " + formula.toSimpleLaTeXString() + "\n");
				imagedAreaManager.appendPlainStringAsNewLine(counter + ": 扩展合取范式 ");
				imagedAreaManager.appendLaTeXStringToLastLine(formula.toSimpleLaTeXString());
				imagedAreaManager.appendPlainStringToLastLine("，变量集 ");
				imagedAreaManager.appendLaTeXStringToLastLine("\\{" + variableSetString + "\\}");
				...
				for (ExpandNFStep step : stepList) {
					...
					textAreaManager.appendContent("\tExpand " + formula.toSimpleLaTeXString() + "[" + step.getFormulaBinaryCodeString() + "] to max terms: " + step.getResultCodesNamedLaTeXString(type) + "\n");
		...
		private boolean checkNormalFormula(String variableSetString, List<String> laTeXStringList, boolean isDNF) {
			...
			if (isDNF) {
				for (String laTeXString : laTeXStringList) {
					...
					if (simpleform == null) {
						...
						JOptionPane.showMessageDialog(inputManager.getDialog(), "输入的字符串 " + formula.toSimpleLaTeXString() + "不是简单合取式，无法扩展为主析取范式!", "离散数学基础例题习题展示", JOptionPane.WARNING_MESSAGE);
						return false;
					}
				}
				DisjunctiveNormalFormula disjnormform = DisjunctiveNormalFormula.convert(formula);
				...
				for (String laTeXString : laTeXStringList) {
					...
					if (simpleform == null) {
						JOptionPane.showMessageDialog(inputManager.getDialog(), "输入的字符串 " + formula.toSimpleLaTeXString() + "不是简单析取式，无法扩展为主合取范式!", "离散数学基础例题习题展示", JOptionPane.WARNING_MESSAGE);
						return false;
					}
				}
				ConjunctiveNormalFormula conjnormform = ConjunctiveNormalFormula.convert(formula);
			...
			if (isDNF) {
				JOptionPane.showMessageDialog(inputManager.getDialog(), "输入的公式都是合法的简单合取式，可以扩展为主析取范式！", "离散数学基础例题习题展示", JOptionPane.WARNING_MESSAGE);
			} else {
				JOptionPane.showMessageDialog(inputManager.getDialog(), "输入的公式都是合法的简单析取式，可以扩展为主合取范式！", "离散数学基础例题习题展示", JOptionPane.WARNING_MESSAGE);
			}
			return true;
		}
