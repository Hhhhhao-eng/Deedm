 ...
 *
 */
public class GenRepCombUIManager extends JPanel {
	...
	public GenRepCombUIManager(PlainTextAreaManager plainManager, ImagedTextAreaManager imageManager) {
	...
	public boolean checkInput() {
		...
		String message = "从 " + baseSet.length() + " 个元素选 " + length + " 个允许重复的组合";
		...
		feedbackAnimator.appendPlainStringAsNewLine("从：");
		feedbackAnimator.appendLaTeXStringToLastLine(startString);
		feedbackAnimator.appendPlainStringToLastLine(" 开始生成" + message + "的 ");
	...
	private JPanel createButtonPanel() {
		...
		JButton exampleButton = new JButton("问题8.40");
		exampleButton.setActionCommand("example8_40");			
		exampleButton.addActionListener(listener);	// 添加事件监听器
		buttonPanelTwo.add(exampleButton);
		
		exampleButton = new JButton("例子8.41");
		exampleButton.setActionCommand("example8_41");			
		exampleButton.addActionListener(listener);	// 添加事件监听器
		buttonPanelTwo.add(exampleButton);
	...
	private class SimpleListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent evt) {
			...
			if (command.equals("ok")) {
				...
				if (success) {
					JOptionPane.showMessageDialog(MainSwingFrame.getMainFrame(), "输入合法，可以生成组合！", "离散数学基础例题习题展示", JOptionPane.WARNING_MESSAGE);
				}
				feedbackAnimator.update();
				...
				clearInput();
			} else if (command.equals("example8_40")) {
				feedbackAnimator.clearContent();
				clearInput();
				baseSetField.setText("1, 2, 3, 4, 5, 6, 7, 8");
				lengthField.setText("5");
				startField.setText("13458");
				numberField.setText("20");
				checkInput();
				feedbackAnimator.update();
			} else if (command.equals("example8_41")) {
				feedbackAnimator.clearContent();
				clearInput();
				baseSetField.setText("1, 2, 3, 4, 5, 6, 7, 8");
				lengthField.setText("5");
				startField.setText("13478");
				numberField.setText("20");
				checkInput();
				feedbackAnimator.update();
		...
		public boolean doCountString() {
			...
			feedbackAnimator.appendPlainStringAsNewLine("从：");
			feedbackAnimator.appendLaTeXStringToLastLine(startString);
			feedbackAnimator.appendPlainStringToLastLine(" 开始输出" + message + "的 ");
			feedbackAnimator.appendLaTeXStringToLastLine("" + number);
			feedbackAnimator.appendPlainStringToLastLine(" 个串！");
			...
			imagedAreaManager.appendPlainStringAsNewLine("    从：");
			imagedAreaManager.appendLaTeXStringToLastLine(startString);
			imagedAreaManager.appendPlainStringToLastLine(" 开始输出" + message + "的 ");
			...
			if (displayCounter <= 0) {
				...
				imagedAreaManager.appendLaTeXStringToLastLine(startString);
				imagedAreaManager.appendPlainStringToLastLine(" 不在生成的组合之中！ ");
			}
			
			textAreaManager.appendContent("\tThere are " + totalCounter + " string(s) for " + length + " combinations that repetition allowed on the set B (element number = " + baseSet.length() + ")\n");
			...
			message = "C(" + baseSet.length() + "-1+" + length + ", " + length + ") = C(" + (baseSet.length()-1+length) + ", " + length + ")";
			imagedAreaManager.appendPlainStringToLastLine(" 允许重复的组合总共有 ");
