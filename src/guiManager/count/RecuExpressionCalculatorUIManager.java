...
import counting.RecuExpressionCalculator;
 ...
 *
 */
public class RecuExpressionCalculatorUIManager extends JPanel {

	private static final long serialVersionUID = 1L;

	private static final int width = Math.max(2* MainSwingFrame.screenWidth/5, 1024)*(MainSwingFrame.screenResolution/96);
	private static final int height = Math.max(1*MainSwingFrame.screenHeight/6, 400)*(MainSwingFrame.screenResolution/96);
	...
	private JTextField inputAField = null;
	private JTextField inputBField = null;
	...
	public RecuExpressionCalculatorUIManager(PlainTextAreaManager plainManager, ImagedTextAreaManager imageManager) {
	...
	public boolean setInputs(String input, String inputN, String inputA, String inputB) {
		inputField.setText(input);
		inputNField.setText(inputN);
		inputAField.setText(inputA);
		inputBField.setText(inputB);
		feedbackAnimator.appendPlainStringAsNewLine("递推表达式 = ");
	...
	private JPanel createInputSetPanel() {
		...
		// 设置该窗格使用的带标题的边框
 		inputPanel.setBorder(BorderFactory.createTitledBorder("输入进行计算的递推表达式（支持初始条件为一个或两个未知数的递推计算，初始条件未输入时使用默认值1，支持大数结果）"));

 		int inputWidth = 50;
		// 创建一个标签用于提示输入集合信息
 		JPanel tempPanel = new JPanel();
 		tempPanel.setLayout(new FlowLayout(FlowLayout.CENTER, 15, 15));
		JLabel promptLabel = new JLabel("递推表达式(E)：", JLabel.RIGHT);
		// 设置选择该标签的快捷键，这使得用户按Alt+E可马上定位到输入姓名的文本字段
		...
		int initialConditionWidth = 15;
		// 项数n
		// 创建一个标签用于提示输入集合信息
		tempPanel = new JPanel();
		tempPanel.setLayout(new FlowLayout(FlowLayout.CENTER, 15, 15));
		promptLabel = new JLabel("项数n(N)：", JLabel.RIGHT);
		// 设置选择该标签的快捷键，这使得用户按Alt+n可马上定位到输入姓名的文本字段
		promptLabel.setDisplayedMnemonic('n');	
		// 创建输入集合元素的文本字段，缺省宽度为15, 但这不意味着只能输入15个字符
		inputNField = new JTextField(initialConditionWidth);
		// 设置刚才创建的标签与nameField相关联（即用于提示该字段所要输入的信息）
		...
		// 创建一个标签用于提示输入集合信息
 		tempPanel = new JPanel();
 		tempPanel.setLayout(new FlowLayout(FlowLayout.CENTER, 15, 15));
		promptLabel = new JLabel("初始条件a(A)：", JLabel.RIGHT);
		// 设置选择该标签的快捷键，这使得用户按Alt+A可马上定位到输入姓名的文本字段
		promptLabel.setDisplayedMnemonic('a');	
		// 创建输入集合元素的文本字段，缺省宽度为15, 但这不意味着只能输入15个字符
		inputAField = new JTextField(initialConditionWidth);
		// 设置刚才创建的标签与nameField相关联（即用于提示该字段所要输入的信息）
		promptLabel.setLabelFor(inputAField);
		// 将标签与文本字段加入到临时的窗格
		tempPanel.add(promptLabel);
		tempPanel.add(inputAField);
		inputPanel.add(tempPanel);
		...
		// 创建一个标签用于提示输入集合信息
 		tempPanel = new JPanel();
 		tempPanel.setLayout(new FlowLayout(FlowLayout.CENTER, 15, 15));
		promptLabel = new JLabel("初始条件b(B)：", JLabel.RIGHT);
		// 设置选择该标签的快捷键，这使得用户按Alt+B可马上定位到输入姓名的文本字段
		promptLabel.setDisplayedMnemonic('b');	
		// 创建输入集合元素的文本字段，缺省宽度为15, 但这不意味着只能输入15个字符
		inputBField = new JTextField(initialConditionWidth);
		// 设置刚才创建的标签与nameField相关联（即用于提示该字段所要输入的信息）
		promptLabel.setLabelFor(inputBField);
		// 将标签与文本字段加入到临时的窗格
		tempPanel.add(promptLabel);
		tempPanel.add(inputBField);
		inputPanel.add(tempPanel);		
	...
	private class SimpleListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent evt) {
			...
			if (command.equals("ok")) {
				...
				String result = null;
				if (success) {
					 result = RecuExpressionCalculator.executeRecuExpression(inputField.getText(), inputAField.getText(), inputBField.getText(), inputNField.getText());
					if (!result.equals("error")) JOptionPane.showMessageDialog(dialog, "输入的组合表达式符合要求，可以进行运算！", "离散数学基础例题习题展示", JOptionPane.WARNING_MESSAGE);
					else JOptionPane.showMessageDialog(dialog, "输入的递推表达式有误，请检查！", "离散数学基础例题习题展示", JOptionPane.WARNING_MESSAGE);
				}
		...
		private boolean inputValidation() {
			...
			String inputA = inputAField.getText();
			String inputB = inputBField.getText();
			
			// a和b没有输入时，使用默认值1
			if (null == inputA || "".equals(inputA.trim())) {
				inputAField.setText("1");
			}
			if (null == inputB || "".equals(inputB.trim())) {
				inputBField.setText("1");
			}
			
			
			Matcher matcher = EXPRESSION_PATTERN.matcher(input);
			try {
				Integer.parseInt(inputNField.getText());
			} catch (Exception exc) {
				JOptionPane.showMessageDialog(dialog, "输入的递推次数n不是合法整数", "离散数学基础例题习题展示", JOptionPane.WARNING_MESSAGE);
				return false;
			}
			
			try {
				Float.parseFloat(inputAField.getText());
				Float.parseFloat(inputBField.getText());
			} catch (Exception exc) {
				JOptionPane.showMessageDialog(dialog, "输入的初始条件a和b不是合法的整数或小数", "离散数学基础例题习题展示", JOptionPane.WARNING_MESSAGE);
				return false;
			}
		...
		public boolean doCombCalculatorDisplay() {
			...
			// 对输入的表达式、初始条件、递推次数的合法性进行校验，校验不通过则不进行后续运算
			if (!inputValidation()) {
				return true;
			}
			
			// 计算结果
			String result = RecuExpressionCalculator.executeRecuExpression(input, inputAField.getText(), inputBField.getText(), inputNField.getText());
			...
			imagedAreaManager.appendPlainStringAsNewLine("    ");
			imagedAreaManager.appendLaTeXStringToLastLine("a_" + inputNField.getText() + " = " + result + "\\quad\\quad\\quad\\quad");	
			
			// 右边区域输出样式
			textAreaManager.appendContent(counter + " : expression = " + input + "\n");
			textAreaManager.appendContent("\t");
			textAreaManager.appendContent("a_" + inputNField.getText() + " = " + result + "\t");
