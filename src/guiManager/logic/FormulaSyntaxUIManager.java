...
import guiManager.MainGUIManager;
					...
					try {
						...
						if (formula != null) {
							...
							if (treeBox.isSelected()) {
								...
								String pngFileName = Configuration.dataFilePath + "test.png";
								FormulaASTGraph ast = FormulaASTGraph.createASTGraph(formula, "PropAST");
								try {
									textAreaManager.appendContent("flag1 is here\t");
									PrintWriter writer = new PrintWriter(dotFileName);
									textAreaManager.appendContent("flag2 is here\t");
									ast.simplyWriteToDotFile(writer);
									textAreaManager.appendContent("flag3 is here\t");
									boolean success = MainGUIManager.generatePNGFile(dotFileName, pngFileName, true);
									textAreaManager.appendContent("flag4 is here\t");
									if (success) {
