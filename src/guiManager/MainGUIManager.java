...
import guiManager.algebra.BinaryOperatorUIManager;
import guiManager.algebra.BoolAlgebraUIManager;
import guiManager.algebra.GroupPermUIManager;
import guiManager.algebra.GroupUmUIManager;
import guiManager.algebra.LatticeUIManager;
import guiManager.count.CombCalculatorUIManager;
import guiManager.count.CountFunctionUIManager;
import guiManager.count.CountIntegerUIManager;
import guiManager.count.CountEquationSolverUIManager;
import guiManager.count.CountStringUIManager;
import guiManager.count.ExprCalculatorUIManager;
import guiManager.count.GenCombinationUIManager;
import guiManager.count.GenPermutationUIManager;
import guiManager.count.GenRepCombUIManager;
import guiManager.count.RecuExpressionCalculatorUIManager;
import guiManager.graph.GraphTravelUIManager;
import guiManager.graph.HuffmanTreeUIManager;
import guiManager.graph.ShortestPathUIManager;
import guiManager.graph.SpanningTreeUIManager;
import guiManager.graph.SpecialGraphUIManager;
import guiManager.graph.TreeTravelUIManager;
import guiManager.logic.EquivCalculusCheckUIManager;
import guiManager.logic.FormulaSyntaxUIManager;
import guiManager.logic.FormulaTruthValueUIManager;
import guiManager.logic.NormalFormulaCalculationUIManager;
import guiManager.logic.NormalFormulaExpansionUIManager;
import guiManager.logic.ReasonArgumentCheckUIManager;
import guiManager.logic.FormulaTruthTableUIManager;
import guiManager.setrelfun.EquivalenceUIManager;
import guiManager.setrelfun.FunctionUIManager;
import guiManager.setrelfun.PartialOrderUIManager;
import guiManager.setrelfun.RelationClosureUIManager;
import guiManager.setrelfun.RelationOperationUIManager;
import guiManager.setrelfun.RelationPropertyUIManager;
import guiManager.setrelfun.SetExprOperationUIManager;
import guiManager.setrelfun.SetOperationUIManager;
...
public class MainGUIManager {
	...
	public static boolean generatePNGFile(String dotFileName, String graphFileName, boolean isTree) {
		...
		try {
			// 1. 创建临时文件
			tempDotFile = File.createTempFile("graphviz_", ".dot");

			// 2. 读取原始文件内容，修复图片路径
			String content = new String(java.nio.file.Files.readAllBytes(
					java.nio.file.Paths.get(dotFileName)), "UTF-8");

			// 修复图片路径：将 .\img\ 替换为绝对路径
			String imgAbsolutePath = new File(dotFileName).getParentFile().getParent() +
					File.separator + "img" + File.separator;
			content = content.replace("image=\".\\img\\",
					"image=\"" + imgAbsolutePath.replace("\\", "\\\\"));

			// 3. 写入临时文件
			java.nio.file.Files.write(tempDotFile.toPath(), content.getBytes("UTF-8"));

			// 4. 使用临时文件执行命令
			args = new String[]{
					dotProgram,
					"-K" + (isTree ? Configuration.treeVizLayout : Configuration.graphVizLayout),
					"-T" + graphFileFormat,
			...
			try (java.util.Scanner scanner = new java.util.Scanner(p.getErrorStream())) {
			...
			if (exitCode != 0) {
				errorMessage = "dot.exe执行失败: " + errorOutput.toString();
				return false;
			}
			...
			buffer.append("\n");
			buffer.append("Exception Message: ").append(exc.getMessage());
			errorMessage = buffer.toString();
			return false;
		} catch (IOException exc) {
			StringBuffer buffer = new StringBuffer();
			buffer.append("Error: Throws IOException during the execution of dot.exe\n");
			buffer.append("Exception Message: ").append(exc.getMessage());
			errorMessage = buffer.toString();
			return false;
		} finally {
			// 5. 清理临时文件
			if (tempDotFile != null && tempDotFile.exists()) {
				tempDotFile.delete();
			}
		}
	}
